...
# Script to verify database objects in Azure SQL Database
# This script runs verify_objects.sql to check for stored procedures and triggers
...
az sql server firewall-rule create --resource-group $RESOURCE_GROUP --server $SQL_SERVER_NAME --name AllowVerifyIP --start-ip-address $MY_IP --end-ip-address $MY_IP --output none || \
az sql server firewall-rule update --resource-group $RESOURCE_GROUP --server $SQL_SERVER_NAME --name AllowVerifyIP --start-ip-address $MY_IP --end-ip-address $MY_IP --output none

echo "====== Verifying Database Objects ======"
echo "Running SQL verification script..."
echo ""
